...
import { getCurrentWorkspace, getMembers } from "@/data/ritx/workspaceState";
  ...
  return (
    ...
    <div className="space-y-4">
      ...
      <div ref={certRef} className="space-y-3">
        {(members.length > 0 ? members : [{ id: "self", name: team.teamName, class: "" }]).map((m) => (
          <Card key={m.id} className="p-8 border-2 border-amber-200 bg-gradient-to-br from-white via-amber-50/40 to-white text-center relative overflow-hidden">
            ...
            <div className="absolute top-3 right-3 text-xs text-muted-foreground">{workspace?.code ?? team.teamCode}</div>
            <div className="text-xs uppercase tracking-widest text-muted-foreground mt-6">Certificate of Achievement</div>
            <div className="text-3xl md:text-4xl font-bold mt-3 bg-gradient-to-r from-amber-600 to-orange-600 bg-clip-text text-transparent">{m.name}</div>
            <div className="text-sm text-muted-foreground mt-1">Team {workspace?.name ?? team.teamName}{"class" in m && m.class ? ` · Class ${m.class}` : ""}</div>
