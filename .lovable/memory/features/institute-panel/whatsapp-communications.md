...
**Automated Alerts**: clean segmented toggle list (modeled on teacher `NotificationPreferences`) split into TWO segment cards the institute manages independently — **Teachers** (timetable updates, substitution, exam updates, syllabus lag) and **Students** (exam reports, exam timetable, general announcements). Student alerts are delivered to the parent's phone (previews read "Dear Parent,") but are labelled Students so teacher vs student alerts toggle separately. No attendance/holiday/schedule-change alerts. Each alert is a compact row (title + description + Switch); the WhatsApp-style `MessagePreview` is **hidden by default and expands inline via a per-row "Preview" button** (NOT always-visible). Toggles persist in `localStorage` key `institute_whatsapp_alert_toggles`; preview open/closed state is ephemeral local state.
