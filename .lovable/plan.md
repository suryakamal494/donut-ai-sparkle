...
- [x] **Chapters QA** — fully rewritten (672 lines). Section E now matches real architecture: full-page 3-step flow (`configure → review → done`) at `/practice` route, all-bands-at-once generation, common + per-band instructions, remove/regenerate, per-band & assign-all flows. C1 now quotes real `reportColors.ts` thresholds (75/50/35) and tells testers to trust code over UI tooltips. Depth pass applied to every scenario.
- [x] **Exams QA** — fully rewritten (723 lines). Depth pass on every scenario: variations in "What to try" (network throttling, rapid clicks, URL manipulation, multi-row checks) and failure shapes + severity in "Expected". Subject-scoping language (B section) preserved and strengthened.

All three Teacher Reports QA documents are now complete with correctness + depth.
